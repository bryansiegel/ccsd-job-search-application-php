...
//             '254' => '442', '442' => '254',    # cortez/diaz				diaz/cortez
    ...
    # New connection
    $zoneSearch = mysql_connect('trmysql.ccsd.net', 'httpd', 'H!2m;Leg+z%ZP7KC');
...
/*
    ...
    else {$lookupTable = '2012zoning.zoning2425_ic';}
*/
    
    if($year == '2025') {$lookupTable = '2012zoning.zoning2425_2025new04';}
    else {$lookupTable = '2012zoning.zoning2526_2025new03';}
...
$mongo = new MongoDB\Driver\Manager('mongodb://commsUnitUser:ekhbwn6CbRET5eBW@mongodb1.ccsd.net:27017,mongodb2.ccsd.net:27017,mongodb3.ccsd.net:27017/?replicaSet=ccsd-mongo-apps&tls=true&tlsAllowInvalidHostnames=true&authSource=ccsdnet');
        $query = new MongoDB\Driver\Query(['TXTSITECODE' => $loc],['limit' => 1]);
        $row = get_object_vars($mongo->executeQuery('ccsdnet.locations', $query)->toArray()[0]);

		// trim left zero
		$row['TXTSITECODE'] = ltrim($row['TXTSITECODE'], '0');

		$school = array( 'TXTSITECODE'=>$row['TXTSITECODE'], 'TXTSITENAME'=>$row['TXTSITENAME'], 'TXTPHONE'=>$row['TXTPHONE'], 'TXTSITEWEB' => $row['TXTSITEWEB'] );
	//}
