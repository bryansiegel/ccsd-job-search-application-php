...
function zoning_lookup($house = null, $pd = null, $street = null, $uniqueid = null, $year = null) {
    ...
    if($year==2017) {
        $sister_schs = array(
            ...
            '285' => '379', '379' => '285',    # walker/smalley			smalley/walker
...
function getSisterSchoolInfo($loc) {
	...
	// conn settings
	$conn = $_dB_cuserdb1 = ocilogon("essproj_app", "ug0tLukk3", "(DESCRIPTION =(LOAD_BALANCE = ON)(FAILOVER = ON)(ADDRESS = (PROTOCOL = TCP)(HOST = mc01-scan.cis.ccsd.net)(PORT = 1521))(CONNECT_DATA = (SERVER = DEDICATED)(SERVICE_NAME = TRWEB)(FAILOVER_MODE = (TYPE = SELECT)(METHOD = BASIC)(RETRIES = 20)(DELAY = 1))))");
	if (!$conn) {
	    $e = oci_error();
	...
	$query = "SELECT txtsitecode, txtsitename, txtphone 
				FROM idm_views.person_registry_location_vw 
