...
function zoning_lookup($house = null, $pd = null, $street = null, $uniqueid = null, $year = null) {
    ...
    */
    ...
    #handle the year
    #NOTE: YEARLY UPDATE
    if($year == '2024') {$lookupTable = '2012zoning.zoning2324_ic';}
    else {$lookupTable = '2012zoning.zoning2425_2024';}
...
function getSisterSchoolInfo($loc) {
	...
	//$mongo = new \MongoClient('mongodb://radUser:4n75GePn@206.194.10.191:27017,206.194.10.192:27017/?replicaSet=TRrep&authSource=radprod');
	//$database = $mongo->selectDB("radprod");
	//$mongo = new \MongoClient('mongodb://commsUnitUser:ekhbwn6CbRET5eBW@mongodb1.ccsd.net:27017,mongodb2.ccsd.net:27017,mongodb3.ccsd.net:27017/?replicaSet=ccsd-mongo-apps&tls=true&tlsAllowInvalidHostnames=true&authSource=locationdataprod');
$mongo = new MongoDB\Driver\Manager('mongodb://commsUnitUser:ekhbwn6CbRET5eBW@206.194.10.191:27017,206.194.10.192:27017/?replicaSet=TRrep&authSource=locationdataprod');
	$database = $mongo->selectDB("locationdataprod");
	
	$collection = $database->locations;

	$where = array('TXTSITECODE' => $loc);
    $row = $collection->findOne($where);
